...
export async function POST(request: Request) {
  ...
  try {
    return withMobileCors(request, NextResponse.json({ tour: await createAndStartTourForCurrentDriver() }));
  } catch (error) {
    ...
    return withMobileCors(
      request,
      NextResponse.json({ message: "Impossible de demarrer la tournee." }, { status: 500 }),
    );
  }
}
