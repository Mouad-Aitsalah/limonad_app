...
import type { PosSessionDto, SaleDto } from "@/types/operations-dto";
...
export function SessionsTab({ sessions, invoices }: SessionsTabProps) {
  ...
  const sessionInvoices = viewingSession
    ? invoices.filter((invoice) => invoice.posSessionId === viewingSession.id)
    : [];
